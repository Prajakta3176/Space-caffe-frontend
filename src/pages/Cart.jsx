...
export default function Cart() {
    ...
    const headers = {
      ...
      const fetch = async()=>{
        const res = await axios.get(`http://localhost:8080/get-all-cart-items/`,{headers});
    ...
    const handleIncrease = async(foodid)=>{
        ...
        try{
            const res = await axios.patch(`http://localhost:8080/add-food-in-cart/`,{},{headers});
    ...
    const handleDecrease = async(foodid)=>{
        ...
        try{
            const res = await axios.patch(`http://localhost:8080/decrease-quantity-of-item`,{},{headers});
            console.log(res.data);
            fetch();
          }catch(err){
            console.log(err);
    ...
    const handleRemove = async(foodid)=>{
        ...
        try{
            const res = await axios.patch(`http://localhost:8080/remove-food-from-cart`,{},{headers});
            console.log(res.data);
            fetch();
          }catch(err){
            console.log(err);
    ...
    const handlePlaceOrder = async()=>{
        try{
            const res = await axios.patch(`http://localhost:8080/place-order`,{},{headers});
            console.log(res.data);
            fetch();
  ...
  return (
    <>
      ...
      {
        foodData && (foodData.length > 0) && (<div className="min-h-screen px-4 py-15 flex flex-col items-center  text-white">      
            ...
            <div className="absolute inset-0 bg-[#0b0f2f]  backdrop-blur-sm rounded-2xl z-0">
